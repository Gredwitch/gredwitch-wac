...
function ENT:SpawnFunction(p, tr)
	...
	e:SetPos(tr.HitPos + tr.HitNormal + Vector(0,0,90))
	e.Owner = p
	e:Spawn()
	e:Activate()
	e:SetSkin(math.random(0,6))
	e:SetBodygroup(9,(math.random(0,15)))
	return e
end
...
function ENT:PhysicsUpdate(ph)
	...
	if self.rotorRpm > 0.85 and self.rotorRpm < 0.89 and IsValid(self.rotorModel) then
		self.rotorModel:SetBodygroup(0,1)
	elseif self.rotorRpm < 0.85 and IsValid(self.rotorModel) then
		self.rotorModel:SetBodygroup(0,0)
	end
	
	if self.rotorRpm > 0.85 and self.rotorRpm < 0.89 and IsValid(self.OtherRotorModel) then
		self.OtherRotorModel:SetBodygroup(0,1)
	elseif self.rotorRpm < 0.85 and IsValid(self.OtherRotorModel) then
