...
timer.Simple(5,function()
	if not found and not WACFrame then
		...
		elseif SERVER then
			timer.Create("WAC-NotInstalled", 10, 0, function() print("WAC Aircraft is not installed!") )
	...
	if not foundG then
		...
		elseif SERVER then
			timer.Create("GredWAC-NotInstalled", 10, 0, function() print("Grediwtch's Base addon is not installed") )
...
if wac and wac.aircraft then wac.aircraft.spawnCategoryC = "WAC Community" 