...
function ENT:PhysicsUpdate(ph)
	...
	if IsValid(phys) and not self.disabled then
		if self.controls.throttle>0 and self.rotorRpm>0 and phys:GetVelocity():Length() > 1299 and trace.HitPos:Distance( self:LocalToWorld(Vector(0,0,62)) ) > 50  and self:GetSequence() != gearup then
