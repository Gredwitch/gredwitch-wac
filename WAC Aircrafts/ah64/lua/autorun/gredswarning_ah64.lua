...
timer.Simple(5,function()
	if CLIENT then
		...
		WACCommunity=steamworks.ShouldMountAddon(108907015) and steamworks.IsSubscribed(108907015)
		...
		if !WACCommunity then
			GredWACCFrame=vgui.Create('DFrame')
			GredWACCFrame:SetTitle("WAC Community 1 is not installed / enabled. To disable this, open the spawnmenu, open the top right 'Options' tab, click on 'Misc' under the 'Gredwitch's Stuff' subtab and untick the 'Enable pop ups about missing content' checkbox.")
			...
			h:SetSize(x*0.99,y*0.96)
			h:SetAllowLua(true)
			h:OpenURL('https://steamcommunity.com/sharedfiles/filedetails/?id=108907015.html')
		end
	end
end)