...
function ENT:SpawnFunction(ply, tr)
	...
	ent:SetPos(tr.HitPos+tr.HitNormal*16+Vector(0,0,120))
...
function ENT:PhysicsUpdate(ph)
	...
	if self.rotorRpm > 0.8 and self.rotorRpm < 0.89 and IsValid(self.OtherRotorModel) then
	...
	local trace=util.QuickTrace(self:LocalToWorld(Vector(0,0,62)), self:LocalToWorld(Vector(0,0,50)), {self, self.Wheels[1], self.Wheels[2], self.TopRotor})
